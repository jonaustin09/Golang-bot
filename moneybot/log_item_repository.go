...
type LogItemRepository interface {
	CreateRecord(item Item, MessageID int32, username string) (*LogItem, error)
...
func (r GormLogItemRepository) CreateRecord(item Item, MessageID int32, username string) (*LogItem, error) {
	uid, err := uuid.NewV4()
	if err != nil {
		return nil, err
	}
	...
	logItem.TelegramUserID = username
